 ...
 * Author: Aadil Rizvi
 * Date: 6/1/2016
 * https://github.com/arembedded/mq_example/blob/master/mq_example.c
*/
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
...
int main(void){
  ...
  if(status != 0){
    printf("Failed to create thread1 with status = %d", status);
    ASSERT(status == 0);
  }
  ...
  if(status != 0){
    printf("Failed to create thread2 with status = %d", status);
    ASSERT(status == 0);
  }
...
void thread2_main(void) {
  ...
  while(1){
    status = mq_receive(my_mq, (char*)&recv_counter, sizeof(recv_counter), NULL);
    if(status > 0){
      printf("RECVD MSG in thread 2: %d\n", recv_counter);
      counter += 1;
    }
    usleep(exec_period_usecs);
  }
